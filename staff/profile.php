...
if (strlen($_SESSION['sturecmsaid']==0)) {
  ...
  {
    $staffid=$_SESSION['sturecmsaid'];
    $SName=$_POST['staffname'];
  $mobno=$_POST['mobilenumber'];
  $email=$_POST['email'];
  $sql="update tblstaff set StaffName=:staffname,MobileNumber=:mobilenumber,Email=:email where ID=:sid";
     $query = $dbh->prepare($sql);
     $query->bindParam(':staffname',$SName,PDO::PARAM_STR);
     $query->bindParam(':email',$email,PDO::PARAM_STR);
     $query->bindParam(':mobilenumber',$mobno,PDO::PARAM_STR);
     $query->bindParam(':sid',$staffid,PDO::PARAM_STR);
$query->execute();
...
{               ?>
                      <div class="form-group">
                        ...
                        <input type="text" name="staffname" value="<?php  echo $row->StaffName;?>" class="form-control" required='true'>
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        <label for="exampleInputCity1">Staff Registration Date</label>
                         <input type="text" name="" value="<?php  echo $row->StaffRegdate;?>" readonly="" class="form-control">
                      </div><?php $cnt=$cnt+1;}} ?> 
